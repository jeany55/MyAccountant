...
-- TODO: Proper default option for currency dropdown
local ViewingCurrency = "Gold"

local function getHeaderDisplay(amount)
  local L = LibStub("AceLocale-3.0"):GetLocale(private.ADDON_NAME)

  if ViewingCurrency == "Gold" then
    return GetMoneyString(amount, true)
  elseif ViewingCurrency == "Item" then
    return format(L["currency_item"], amount)
  else
    return format(L["currency_currencies"], amount)
  end
end

function MyAccountant:InitializeIncomeFrame()
  local L = LibStub("AceLocale-3.0"):GetLocale(private.ADDON_NAME)
  ...
  LibDD:UIDropDownMenu_Initialize(currencyDropdown, function()
    ...
    goldRow.text = "|cffffff00" .. L["gold"] .. "|r"
    goldRow.value = "Gold"
    goldRow.icon = "Interface\\MoneyFrame\\UI-GoldIcon"
    ...
    local itemsRow = LibDD:UIDropDownMenu_CreateInfo()
    itemsRow.text = L["items"]
    itemsRow.value = "Item"
    itemsRow.func = function()
      LibDD:UIDropDownMenu_SetSelectedValue(currencyDropdown, "Item")
      ViewingCurrency = "Item"
      MyAccountant:UpdateFrame()
    end

    local currencyRow = LibDD:UIDropDownMenu_CreateInfo()
    currencyRow.text = L["currencies"]
    currencyRow.value = "Currencies"
    currencyRow.func = function()
      LibDD:UIDropDownMenu_SetSelectedValue(currencyDropdown, "Currencies")
      ViewingCurrency = "Currencies"
      MyAccountant:UpdateFrame()
    end

    LibDD:UIDropDownMenu_AddButton(goldRow)
    LibDD:UIDropDownMenu_AddButton(itemsRow)
    LibDD:UIDropDownMenu_AddButton(currencyRow)
  end)
...
function MyAccountant:MakeRows(dataTable)
  local rows = {}

  for view, viewData in pairs(dataTable) do
    table.insert(rows, {
      type = "header",
      name = view,
      income = viewData.Gold.Gold.income,
      outcome = viewData.Gold.Gold.income,
    })
    for view.viewData in pairs
  end
end
...
function MyAccountant:UpdateFrame()
  ...
  dataTable = MyAccountant:SummarizeDay(date("*t"), "source")

  local totalSummary = { income = 0, outcome = 0 }
  for _, v in pairs(dataTable) do
    totalSummary.income = totalSummary.income + v.Gold.Gold.income
    totalSummary.outcome = totalSummary.outcome + v.Gold.Gold.outcome
  end

  local net = totalSummary.income - totalSummary.outcome

  totalIncome:SetText(getHeaderDisplay(totalSummary.income))
  totalOutcome:SetText(getHeaderDisplay(totalSummary.outcome))
  totalProfit:SetText(getHeaderDisplay(net))
  ...
  for view, viewData in pairs(dataTable) do
    ...
    for transaction, row in pairs(viewData.Gold.Gold.transactions) do

    end

    table.insert(rows, rowPayload)
  end

  MyAccountant:DrawFrame()

  local drawingParent = true
  ...
  for i = 1, 12 do
    ...
    if drawingParent then
      _G[title]:SetText(currentRow.name)
      _G[incoming]:SetText(GetMoneyString(currentRow.income, true))
      _G[outgoing]:SetText(GetMoneyString(currentRow.outcome, true))
      drawingParent = false
      childIndex = 1
    else
...
function MyAccountant:DrawFrame()
  ...
  local scrollBarUpdateFunction = function()
    if (#rows > 12) then
      FauxScrollFrame_Update(scrollFrame, #rows, 12, 20);
    end

...
local Meta = {
  __index = function(_, k)
    if k.match and k:match("^[A-Z]") and k:match("[^A-Z_]") then
      return Fn
    end
  end,
}
...
CreateFrame = function(frameType, name, parent, template, id)
  ...
  local CreateChild = function()
    return CreateFrame(nil, nil, f)
  end
  local GetTexture = function(name)
    return function()
      if not textures[name] then
        textures[name] = CreateChild()
      end
  ...
  ---@class Frame
  f = setmetatable({
    SetScript = function(_, k, v)
      scripts[k] = v
    end,
    GetScript = function(_, k)
      return scripts[k]
    end,
    HasScript = function(_, k)
      return not not scripts[k]
    end,
    RegisterEvent = function(_, k)
      events[k] = true
    end,
    UnregisterEvent = function(_, k)
      events[k] = nil
    end,
    ...
    SetPoint = function(_, ...)
      ...
      if n == 1 then
        rel, relPoint, x, y = parent, point, 0, 0
      end
      if n == 3 and type(rel) == "table" then
        x, y = 0, 0
      end
      if n == 3 and type(rel) == "number" then
        x, y, rel, relPoint = rel, relPoint, parent, point
      end
      table.insert(points, { point, rel, relPoint, x, y })
    end,
    GetPoint = function(_, k)
      return unpack(points[k])
    end,
    ...
    FireEvent = function(_, e, ...)
      if scripts.OnEvent and events[e] then
        scripts.OnEvent(f, e, ...)
      end
    end,
    FireUpdate = function()
...
string.split = function(del, str, n)
  ...
  for a, b in string.gmatch(str, "([^" .. del .. "]*)" .. del .. "([^" .. del .. "]*)") do
    if i == 0 or a:len() > 0 or b:len() == 0 then
      push(a)
    end
    if i == 1 or b:len() > 0 then
      push(b)
    end
  end
  if i == 0 then
...
end --[[@as fun(str: string, chars: string?): string]]
...
UnitName = function(u)
  local unit, realm = strsplit("-", u)
  return unit, realm ~= GetRealmName() and realm or nil
end
      ...
      if test.passed then
        ...
        file:write(
          string.format(
            ...
            escapeXML(test.error or "Test failed"),
            test.error or "Test failed"
          )
        )
        file:write("    </testcase>\n")
      end
